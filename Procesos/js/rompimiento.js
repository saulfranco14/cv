 ...
 */
  ...
  */
   ...
   */
   ...
   const GATEWAY_alumno_HAS_ALUMNO = "../zend_gateway/index.php";
   ...
   */
   ...
   var CLASE_alumno_HAS_ALUMNO = 'alumno_has_alumno';
	...
	$(document).ready(
      ...
      */
       ...
       */
       ...
       function listaralumno_has_Alumno()

      {
        ...
        $.ajax({
          ...
          data : peticionJSON,
          dataType : 'json',
          url : GATEWAY_alumno_HAS_ALUMNO,
          success : function(jsonRespuesta, estatusRespuesta, jqXHR)
          {
            exitolistaralumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR);
          },
        ...
        * @returns {void}
        */
        function exitolistaralumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          ...
          indice = 0;
          jsonalumno_has_Alumno = jsonRespuesta.result;
          $('#tbodyRompimiento > tr').remove();
          if(jsonalumno_has_Alumno.length > 0){
            htmlNuevo = '';
            for(indice=0; indice<jsonalumno_has_Alumno.length; indice++){
              objetoalumno_has_Alumno = jsonalumno_has_Alumno[indice];
              htmlNuevo += '<tr>';
                htmlNuevo += '<td>';
                ...
                htmlNuevo += '</td>';
                htmlNuevo += '<td class="hidden-xs>' + objetoalumno_has_Alumno.a + '</td>';
                htmlNuevo += '<td class="hidden-xs>' + objetoalumno_has_Alumno.b + '</td>';
                htmlNuevo += '<td>' + objetoalumno_has_Alumno.c + '</td>';
                htmlNuevo += '</tr>';
            }
            $('#tbodyRompimiento').append(sanitizarHTML(htmlNuevo));
          }else{
            mostrarVentanaModal('No hay alumno ni alumnos');
          }
          switch (accion){
          case 'buscar':
      ...
      */
      ...
      function seleccionoRegistro(idBoton)
      {
        ...
        switch(queAccion){
          case 'editar':
            editaralumno_has_Alumno(queRegistro);
            break;
          case 'borrar':
      ...
      */
      ...
      function editaralumno_has_Alumno(indiceEscogido)
      {
        recuperado = true;
        objetoalumno_has_Alumno = jsonalumno_has_Alumno[indiceEscogido];
        $('#tabsMenu a[href="#divTabFormularioalumno_has_Alumno"]').tab('show');
        $('#alumno_Id_Alergia').val(objetoalumno_has_Alumno.a);
        $('#Alumno_Id_Alumno').val(objetoalumno_has_Alumno.b);
        $('#Fecha_Alergia').val(objetoalumno_has_Alumno.c);
        comboalumno();
      ...
      */
      ...
      // checamos primero, si existio un error personalizaod:
       ...
       $('#Id_Alergia').append('<option value="'+ objetoalumno.a + '">'+ objetoalumno.b + '</option>');
      }
      }else{
       $('#Id_Alergia').append('<option value="x">No hay alumno </option>');
      }
      if (recuperado) {
       $('#Id_Alergia').val(objetoalumno_has_Alumno.d).attr('selected','selected');
      }
      }
      ...
      */
      ...
      if (recuperado) {
       $('#Id_Alumno').val(objetoalumno_has_Alumno.d).attr('selected','selected');
      }
      }
      ...
      */
      ...
      function agregaralumno_has_Alumno()
      {
        objetoalumno_has_Alumno = null;
        $('#tabsMenu a[href="#divTabFormularioalumno_has_Alumno"]').tab('show');
        $('#alumno_Id_Alergia').val('');
      ...
      */
      ...
      function guardaralumno_has_Alumno()
      {
        objetoalumno_has_Alumno = {
          a: $('#alumno_Id_Alergia').val(),
          b: $('#Alumno_Id_Alumno').val(),
          c: $('#Fecha_Alergia').val()
        };
        if (objetoalumno_has_Alumno.a == 0) {
          //insertar Grupo
          accion = 'insertar';
        }else{
          //actualizar Grupo
       ...
       peticionJSON = JSON.stringify({
          ...
          'method' : accion,
          'clase' : CLASE_alumno_HAS_ALUMNO,
          'Params' : [objetoalumno_has_Alumno]
        });
        $.ajax({
          method : 'POST',
          timeout : 30000,
          ...
          success : function(jsonRespuesta, estatusRespuesta, jqXHR)
          {
            exitoGuardadoalumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR);
          },
          error : function(jqXHR, estatusError, textoError)
          {
            mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        */
        ...
        function exitoGuardadoalumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR)
        {
            ...
            switch(accion){
              ...
              case 'actualizar':
                if(jsonRespuesta.result == 1) {
                  //si se actualizo
                  mostrarVentanaModal('Grupo  ' + objetoalumno_has_Alumno.a + '   Actualizado');
                }else{
                  //no se actuzalizo
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBorraralumno_has_Alumno"]').tab('show');
          objetoalumno_has_Alumno = jsonalumno_has_Alumno[indiceEscogido];
          htmlNuevo = 'ID: <strong>' + objetoalumno_has_Alumno.a + '</strong><br/>Nombre: <strong>' + objetoalumno_has_Alumno.b + '</strong>';
          $('#pDatosalumno_has_Alumno').html(sanitizarHTML(htmlNuevo));
         }

         function borrarGrupo()
         {
          objetoalumno_has_Alumno = { a:objetoalumno_has_Alumno.a };
          peticionJSON = JSON.stringify({
            'Id' : generarID(),
            'method' : 'borrar',
            'clase' : CLASE_alumno_HAS_ALUMNO,
          ...
          $.ajax({
            ...
            success : function(jsonRespuesta, estatusRespuesta, jqXHR)
            {
              exitoBorradoalumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR);
            },
            error : function(jqXHR, estatusError, textoError)
            {
              mostrarErrorJSON(jqXHR, estatusError, textoError);
      ...
      /**
        ...
        * @returns {void}
        */
        function exitoBorradoalumno_has_Alumno(jsonRespuesta, estatusRespuesta, jqXHR)
        {
          //checamoa si existio un error
            ...
            if(jsonRespuesta.result == 1){
              //si se borro:
              mostrarVentanaModal(' alumno y Alumno   ' + objetoalumno_has_Alumno.a + '   Borrado<br />(recuerde: Este borrado no se puede deshacer).');
              
            }else{
              //no se borro:
              mostrarVentanaModal(' alumno y Alumno  '+ objetoalumno_has_Alumno.a + 'No pudo ser borrado.');
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabListaalumno_has_Alumno"]').tab('show');
          listaralumno_has_Alumno();
         }
         /**
         ...
         */
         ...
         {
          $('#tabsMenu a[href="#divTabBuscaralumno_has_Alumno"]').tab('show');
          $('#inputCriterio').val('');
         ...
         * @returns {void}
         */
         function buscaralumno_has_Alumno()
         {
